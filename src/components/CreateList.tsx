...
const CreateList: React.FC = () => {
  ...
  return (
    <Card className="p-10 w-96">
      ...
      <div/>
        <Typography className="text-lg font-bold mb-2">List Items:</Typography>
        {listItems.map((item, index) => (
          <div key={index} className="flex flex-col w-72 gap-3">
            <Input
              label={`Item ${index + 1} Title`}
              value={item.title}
              onChange={(event) => handleItemTitleChange(event, index)}
              className="mr-4"
            />
            <Input
              label={`Item ${index + 1} Content`}
              value={item.content}
              onChange={(event) => handleItemContentChange(event, index)}
              className="mr-4"
            />
          </div>
        ))}
        ...
        <Button color={listTitle ? "blue" : "blue-gray"} onClick={handleCreateList}>
          Create List
        </Button>
      </div>
    </Card>
